  ...
  blocks: [
    {
      id: 'header-1',
      type: 'Header',
      settings: { backgroundColor: '#111827', textColor: '#FFFFFF', sticky: false },
      content: { title: 'Tu Tienda', navLinks: ['Inicio', 'Productos', 'Contacto'] },
    },
    {
      id: 'hero-1',
      type: 'HeroSection',
...
const PublicStorePage = () => {
  ...
  useEffect(() => {
    const fetchStoreData = async () => {
      ...
      setLoading(true);
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/store-settings/${slug}`);
        const settingsData = res.data;

        if (!settingsData || !settingsData.settings) {
          setError('Tienda no encontrada o sin configuración.');
          setLoading(false);
          return;
        }
        ...
        Object.keys(settingsData.settings).forEach((key) => {
          if (key === 'blocks' && Array.isArray(settingsData.settings.blocks)) {
            ...
            typeof settingsData.settings[key] === 'object' &&
            settingsData.settings[key] !== null &&
            !Array.isArray(settingsData.settings[key])
          ) {
        ...
        setError(err.response?.data?.error || 'Error al buscar la tienda.');
        console.error(err);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="min-h-screen w-full flex flex-col items-center justify-center bg-gray-900 text-white">
        <p className="mt-4 text-xl font-semibold">
          No se encontró la configuración de diseño para esta tienda.
        </p>
      </div>
    );
  }
