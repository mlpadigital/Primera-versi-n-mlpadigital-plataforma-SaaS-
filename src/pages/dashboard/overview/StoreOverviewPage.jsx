...
import { Loader2, Copy, ExternalLink, CheckCircle, Eye } from 'lucide-react';
import { motion } from 'framer-motion';
import StorePreview from '../design/StorePreview';
...
const StoreOverviewPage = () => {
  ...
  const fetchStoreData = useCallback(async () => {
    ...
    try {
      ...
      setStoreData({
        name: storeInfo.name,
        slug: settingsInfo?.store_slug || storeId,
      });

      if (settingsInfo && settingsInfo.settings) {
        const mergedSettings = JSON.parse(JSON.stringify(initialDesignSettings));
        Object.keys(settingsInfo.settings).forEach(key => {
            if (key === 'blocks' && Array.isArray(settingsInfo.settings.blocks)) {
                mergedSettings.blocks = settingsInfo.settings.blocks;
            } else if (typeof settingsInfo.settings[key] === 'object' && settingsInfo.settings[key] !== null && !Array.isArray(settingsInfo.settings[key])) {
                mergedSettings[key] = { ...mergedSettings[key], ...settingsInfo.settings[key] };
            } else {
                mergedSettings[key] = settingsInfo.settings[key];
            }
        });
  ...
  if (!storeData) {
    return (
      <div className="text-center py-10">
        <h2 className="text-2xl font-semibold text-red-400">No se encontraron datos de la tienda.</h2>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div>
        <h1 className="text-4xl md:text-5xl font-bold mb-2">Resumen de <span className="text-yellow-400">{storeData.name}</span></h1>
        <p className="text-indigo-200 text-lg">Aquí tienes el enlace público de tu tienda y un resumen de su actividad.</p>
      </div>

      <Card className="glass-effect border-purple-500 shadow-xl">
        <CardHeader>
          <CardTitle className="text-2xl text-yellow-300">Enlace de tu Tienda</CardTitle>
          <CardDescription className="text-indigo-300">Este es el enlace que puedes compartir con tus clientes para que visiten tu tienda.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col sm:flex-row items-center gap-3">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <Button onClick={handleCopyLink} variant="outline" className="text-white hover:bg-white/10 border-purple-400 w-full sm:w-auto">
                {copied ? <CheckCircle className="h-5 w-5 text-green-400" /> : <Copy className="h-5 w-5" />}
                <span className="ml-2">{copied ? 'Copiado' : 'Copiar'}</span>
              </Button>
              <Button asChild className="bg-yellow-400 text-purple-700 hover:bg-yellow-500 font-semibold w-full sm:w-auto">
      ...
      <Card className="glass-effect border-purple-500 shadow-xl">
        <CardHeader>
          <CardTitle className="text-2xl text-yellow-300 flex items-center"><Eye className="mr-3"/>Vista Previa de la Tienda</CardTitle>
          <CardDescription className="text-indigo-300">Así es como los visitantes ven tu tienda en este momento. Los cambios se reflejan al instante.</CardDescription>
        </CardHeader>
        <CardContent>
