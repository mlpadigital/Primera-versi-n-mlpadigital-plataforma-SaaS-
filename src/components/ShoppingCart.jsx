      ...
      {isCartOpen && (
        ...
        >
          <motion.div
            ...
            className="absolute right-0 top-0 h-full w-full max-w-md bg-slate-900/80 backdrop-blur-lg shadow-2xl flex flex-col text-white"
              ...
              {cartItems.length === 0 ? (
                ...
                cartItems.map(item => (
                  <div key={item.variant.id} className="flex items-center gap-4 bg-white/5 p-3 rounded-lg">
                    <img src={item.product.image || 'https://via.placeholder.com/80'} alt={item.product.title} className="w-20 h-20 object-cover rounded-md" />
                    <div className="flex-grow">
                      <h3 className="font-semibold">{item.product.title}</h3>
                      <p className="text-sm text-gray-300">{item.variant.title}</p>
                      <p className="text-sm text-primary font-bold">
                        {item.variant.sale_price_in_cents_formatted ?? item.variant.price_in_cents_formatted}
                      </p>
                    </div>
